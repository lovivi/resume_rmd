...
ukbiobank 数据（50w 人+80w 遗传位点+2000表型），结合galaxy系统开发并优化了自动化分析流程，包含数据提取，预处理，清洗，分组，关联分析，建模分析等。构建了一套ukb数据的表型-基因型关联工具。对AD和MCI进行了关联分析，构建了风险预测模型和评价指标，结合院内数据写了深自然本子（未中）。<br>对早期型COPD进行关联分析，并发表论文。
